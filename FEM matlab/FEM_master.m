...
%% %%%%%%%%%%%%%%%%%% define model %%%%%%%%%%%%%%%%%%%%%%%%%
W  = 10;        % Unit zell widht and height [mm]
R  = 5;        % Radius of hole [mm]
h  = 1;         % model thickness
...
D_forced = 1; %Forced displacement [mm]

%% %%%%%%%% Generate Mesh, Constraints and Loads %%%%%%%%%%%%%%%%%%%%%%%

% generate mesh (node and element tables)
[nodes,elems,ndof] = MeshMaster(meshType,W,R,nc,no);
...
BC(find(nodes(:,2) == W)*2) = 1; % Top nodes
% Forced displacements
Dc = zeros(ndof,1);
Dc(find(nodes(:,2) == W)*2) = D_forced;

% Multivariable constraints
MC(find(nodes(:,1) == W)*2-1) = 0; % =1 if on
...
[eps,sigma_val] = StressCal(elems,nodes,D,E,meshType);
...
if mod(length(x_vec),3) ~= 0 %Rectangular element
    ...
    for i = 1:nGP    
        for j = 1:nGP
            ...
            k = k + B'*Em*B*h*abs(J_det)*W(i)*W(j);
        end
    end
    ...
    for i = 1:3
        ...
        k = k + B'*Em*B*h*(1/2)*abs(J_det)*W(i)*W(i);
    end
end
end
...
function [eps,sigma_val] = StressCal(elems,nodes,D,E,meshType)
% Calculating stresses in the elements
% D = Displacements
...
for i = 1:size(elems,1) % Looping over number of elements
    ...
    for j = 1:size(elems,2) 
        ...
        %B =  BmatMaster(meshType,x_vec,y_vec,xi(j),eta(j));
        [B,~,N] =  BmatMaster(meshType,x_vec,y_vec,xi(j),eta(j));
...
for j = 1:max(elems(:)) % Looping over number of nodes
    ...
    for i = 1:length(elem_nr)
        ...
        node_stresses(:,i) =  sigma_val(:,node_pos(i),elem_nr(i));
    end
    % Averaging the stresses
    node_stress_avg = mean(node_stresses(1:3,:),2);
    % Inserting the stresses back in the global stress matrix
    for i = 1:length(elem_nr)
        sigma_val(:,node_pos(i),elem_nr(i)) = node_stress_avg ;
    end
end
%
...
function res = post_process(elems,nodes,D,Em,meshType)
    ...
    for i = 1:ne
        ...
        for j = 1:4
            ...
            B = BmatMaster(meshType,x,y,c(j,1),c(j,2));
            
            % calculate strain and stress the simple way:
            % (a better way is to extrapolate from the Gauss points)
            eps   = B*d;            
            sigma = Em*eps;
...
function plot_results(elems,nodes,res,plot_mode)
    ...
    switch plot_mode
        ...
        case {'SX','SY','SXY','EX','EY','EXY' 'S1' 'S2' 'Se'} % element-based results
            for i=1:size(elems,1)
