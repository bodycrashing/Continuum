...
D_forced = 0.1; %Forced displacement [mm]
...
function res = post_process(elems,nodes,D,Em,meshType)
    ...
    for i = 1:ne
        ...
        for j = 1:size(elems,2)
            ...
            [B,~,~] = BmatMaster(meshType,x,y,1/scale * c(j,1),1/scale * c(j,2));
    ...
    for i = 1:length(elem_nr)
        ...
        node_stresses(i) =  res.element(elem_nr(i)).Se(node_pos(i));
    end
    % Averaging the stresses
    node_stress_avg = mean(node_stresses);
    % Inserting the stresses back in the global stress matrix
    for i = 1:length(elem_nr)
        res.element(elem_nr(i)).Se(node_pos(i)) = node_stress_avg ;
    end
    end
