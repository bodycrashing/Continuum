...
BC = false(ndof,1);
BC([1 2 3:2*nh+1],1) = 1; % fixed dofs, all others are free
...
%   K     = global structure stiffness matrix
    ...
    % initializing K as sparse for increased speed
    max_nz  = 8*ne;        % max. non-zero entries for Q4 elements
    K       = spalloc(ndof,ndof,max_nz);    % allocate sparse matrix
