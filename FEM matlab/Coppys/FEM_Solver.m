...
% MMP, rev. 4, 26/9-2018.
function [E_unitCell,nu_unitCell] = FEM_Solver(spec)

%% %%%%%%%%%%%%%%%%%% define model %%%%%%%%%%%%%%%%%%%%%%%%%
W  = spec.W/2;        % Unit zell widht and height [mm]
R  = spec.D/2;        % Radius of hole [mm]
h  = 1;         % model thickness

nc = spec.nc;         % no. elements in on hole curve
no = spec.no;         % no. elements in allong diagonal of hole curve
% nc MUST!! be even numbers
nGP = 2;        % no. Gauss points

E  = spec.E;    % Youngs modulus
nu = spec.nu;       % Poissons ratio
