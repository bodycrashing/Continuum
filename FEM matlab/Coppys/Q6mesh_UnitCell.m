...
num_nc = nc+1; % Number of nodes along the cirumfrance of the curve 
num_no = no+1; % Number of nodes 

% Vector containing radiuses

r = linspace(R/W,1,2*num_no); % Vi bygger her dobbelt så mange noder som i Q4 modellen!
% Vector containing node positions on boundary
b = linspace(0,1,ceil(num_nc));

% Creating boundary nodes 
for i = 1:ceil((num_nc)/2)
    if i == 1
        nodes(i,1) = 1;
        nodes(i,2) = 0;
    elseif i == ceil((num_nc)/2)
        nodes(i,1) = 1;
        nodes(i,2) = 1;
    else
        nodes(i,1) = 1;
